<?php
    ...
    function ViewMoviesList(){
      ...
      //  $moviesArray = $moviesmodel->GetMovies();
        $webClient = new WebClient("http://localhost/supinfomoviestore/codesource/");
        $moviesArray = json_decode($webClient->Get_HttpServices(), true);

        foreach ($moviesArray as $key => $movie){
            //print_r ($moviesArray[$key]["year"]);
            $result = $result .
                    "<form method='post' action='Index.php?action=add&id=".$moviesArray[$key]["movie_id"]."'>
                        ...
                        <tr>
                            ...
                            <td style='text-align: center; width: 195px;'>  
                                ...
                                <div style='float: left;margin-left: 30px;'>
                                    <div id='starMain'><img src='../assets/star.png'></div>
           ...
           foreach ($moviesArray as $key => $movie){
            if($movie->movie_id == $movie_id){
                if(intval($movie->stock_quantity) == 0){
                    return "<div style='margin: 10px;''><span style='color: #3D2683; font-size: 22px;'>Out of stock</span></div>";
                }else{
